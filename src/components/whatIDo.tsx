...
const WhatIDo = () => {
  ...
  const highlights = [
    { number: 'Top 5', label: 'University Project Ranking', description: 'Final Year Project recognition' },
    { number: '5+', label: 'Team Members Led', description: 'With personal code completion of 90%' },
    { number: '5+', label: 'Full-Stack Apps', description: 'Including an Ecommerce Solution site' },
    { number: '2+', label: 'Years Experience', description: 'Building modern web applications' },
    { number: 'Top 2', label: 'Graduated with Honours', description: 'in Information Systems from the University of Gondar' },
  ];

  const whatIdo = [
    { icon: '🎨', title: 'Frontend Development', description: 'Build responsive, accessible frontend apps with React + TypeScript + Next.js and React-Native' },
    { icon: '⚙️', title: 'Backend APIs', description: 'Design seamless backend APIs using Node.js, Express, FastAPI and MongoDB, MySQL' },
    { icon: '🤖', title: 'ML & API Integration', description: 'Integrate third-party APIs and ML models into real-world applications' },
    { icon: '🚀', title: 'Deployment & UI', description: 'Deliver clean UI with Tailwind, deploy with Vercel/Render/Netlify/Heroku' },
  ];
        ...
        <div className="space-y-12">
          ...
          >
            <div className="text-center mb-6">
              ...
              <h3 className={`text-3xl font-bold ${isDark ? 'text-[#E6EDF3]' : 'text-gray-900'}`}>What I Do</h3>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {whatIdo.map((item, index) => (
                <div
                  key={index}
                  className={`flex flex-col items-center text-center gap-4 p-6 rounded-xl ${isDark ? 'bg-[#161B22] border-[#30363D] hover:border-[#3B82F6]/50' : 'bg-gray-50 border-gray-200 hover:border-[#3B82F6]/50'} border transition-all duration-300 hover:shadow-lg`}
                >
